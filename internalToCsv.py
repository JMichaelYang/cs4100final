...
class InternalToCSV:
    ...
    def convert(self, internal, filename=None):
        if not filename:
            self.run_count += 1
            filename = f"untitled{self.run_count}.aim"
            logging.debug(f'No filename given; defaulting to <{filename}>')
        logging.info(f'Converting <{filename}> to CSV format...')

        self.tempo = 0
        self.time_signature = None
        self.internal = [ln.strip() for ln in csv.split('\n')]
        self.drop_count = 0
        ...
        coloredLogging.printc(coloredLogging.GREEN,
                              f'Done converting: <{filename}>')
        return out
    ...
    def parse(self, interleaved):
        ...
        if self.drop_count > 0:
            logging.warning(
                f'{self.drop_count} {"Notes" if self.drop_count > 1 else "Note"} dropped')
